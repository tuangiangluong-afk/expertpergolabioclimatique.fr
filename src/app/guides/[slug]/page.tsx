...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const schema = {
        ...
        "image": guide.image ? [`https://www.expertpergolabioclimatique.fr${guide.image}`] : [`https://www.expertpergolabioclimatique.fr/api/og`],
    ...
    const howToSchema = toc.length >= 3 ? {
        ...
        "step": toc.filter((h: any) => h.level === 2).map((h: any, i: number) => ({
            ...
            "name": h.text,
            "text": h.text,
            "url": `https://www.expertpergolabioclimatique.fr/guides/${resolvedParams.slug}#${h.id}`
        }))
    } : null;
